...
def chart_paths(wire_paths)
  ...
  wire_paths.each do |path|
    ...
    distance.times do
      ...
      points << point
      lookups[point] = steps unless lookups[point]
    end
  end
  OpenStruct.new(points: points, lookups: lookups)
end

charted_1 = chart_paths(wire1)
charted_2 = chart_paths(wire2)

lookups_1 = charted_1.lookups
lookups_2 = charted_2.lookups

intersections = charted_1.points & charted_2.points
manhattan_distances = intersections.map { |point| point.x.abs + point.y.abs }
puts "Shortest manhattan distance: #{manhattan_distances.min}"

distances_to_intersections = intersections.map { |point| lookups_1[point] + lookups_2[point] }
puts "Shortest step count to an intersection: #{distances_to_intersections.min}"
